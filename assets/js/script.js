  ...
  $(".search-button").click((event) => {
    ...
    $.ajax({
      ...
      success: function(result) {
        ...
        $(".centered-searchbar").removeClass("centered-searchbar").addClass("top-searchbar");
        $(".results").empty();
        let results = result.query.pages;

        for (var prop in results) {
          $(".results").append("<li><a href='https://en.wikipedia.org/wiki?curid=" + results[prop].pageid + "' target='_blank'><div><h2>" + results[prop].title + "<h3><p>" + results[prop].extract + "</p></div></a></li>")
        }
      }
    })
    .fail((error) => {
